...
<body>
   ...
   <%
                        ...
                        <form action="QuestionsAction.jsp">
                        ...
                        <%
                           ...
                           String table=request.getParameter("table");
                              System.out.println(qid+ " " + table);
                          
                           int i=0;
                           Class.forName("com.mysql.jdbc.Driver");
                           Connection con=DriverManager.getConnection("jdbc:mysql://localhost/onlinelearning","root","root");
                           PreparedStatement query1=con.prepareStatement("SELECT * FROM java  where title='"+title+"'  ");
                           
                           System.out.println(query1);
                           ResultSet rs=query1.executeQuery();
                           System.out.println(rs);
                           System.out.println("SELECT * FROM "+table+" where id='"+qid+"'");
                           while(rs.next())
                           {
                               i++;
                               ...
                               %>
                               <tr style="color: darkgreen;font-size: 20px;font-weight: bold;text-align: left"><td><%=i%>. <%=question%></td></tr>
                               <tr style="color: darkgreen;font-size: 20px;font-weight: bold;text-align: left">
                                    <td>
                                        <input type="hidden" value="<%=answer%>" name="ans<%=i%>" id="ans<%=i%>">
                                        <input type="hidden" value="<%=question%>" name="q<%=i%>" id="q<%=i%>">
                                        <input type="hidden" value="<%=table%>" name="testname" id="testname">
                                        <input type="hidden" value="<%=title%>" name="title" id="title">
                                          <input type="hidden" value="<%=testn%>" name="testn" id="title">
                                        &emsp;&emsp;&emsp;<input type="radio" id="c<%=i%>qq" name="c<%=i%>" value="<%=c1%>" required><%=c1%>
                                    </td>
                               </tr>
                               <tr style="color: darkgreen;font-size: 20px;font-weight: bold;text-align: left">
                                    <td>
                                        &emsp;&emsp;&emsp;<input type="radio" id="c<%=i%>ww" name="c<%=i%>" value="<%=c2%>" required><%=c2%>
                                    </td>
                               </tr>
                               <tr style="color: darkgreen;font-size: 20px;font-weight: bold;text-align: left">
                                    <td>
                                        &emsp;&emsp;&emsp;<input type="radio" id="c<%=i%>ee" name="c<%=i%>" value="<%=c3%>" required><%=c3%>
                                    </td>
                               </tr>
                               <tr style="color: darkgreen;font-size: 20px;font-weight: bold;text-align: left">
                                   <td>
                                       &emsp;&emsp;&emsp;<input type="radio" id="c<%=i%>rr" name="c<%=i%>" value="<%=c4%>" required><%=c4%>
                                   </td>
                               </tr>
                               <%
                           }
                           ...
                           %>
                           <tr align="center"><td colspan="2"><input type="submit" value="Submit Test"></td></tr>
                            </table>
                        </form>
                            </header><!-- .entry-header -->
